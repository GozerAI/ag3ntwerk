...
class TestNexusCsuiteIntegration:
    ...
    async def test_execution_request_flow(self, clean_redis):
        ...
        await asyncio.wait_for(asyncio.gather(csuite_listener(), nexus_sender()), timeout=5.0)
    ...
    async def test_guidance_request_flow(self, clean_redis):
        ...
        await asyncio.wait_for(asyncio.gather(nexus_listener(), csuite_requester()), timeout=5.0)
    ...
    async def test_outcome_reporting_flow(self, clean_redis):
        ...
        await asyncio.wait_for(asyncio.gather(nexus_listener(), csuite_reporter()), timeout=5.0)
    ...
    async def test_learning_sync_flow(self, clean_redis):
        ...
        await asyncio.wait_for(asyncio.gather(nexus_listener(), csuite_syncer()), timeout=5.0)
...
class TestFullFlow:
    ...
    async def test_complete_task_execution_flow(self, clean_redis):
        ...
        async def run_csuite_side():
            ...
            async for message in pubsub.listen():
                if message["type"] == "message":
                    ...
                    events.append(("csuite_received_request", request["request_id"]))
                    ...
                    events.append(("csuite_sent_response", request["request_id"]))
                    ...
                    await redis_client.publish("ag3ntwerk:nexus:learning:sync", json.dumps(sync_data))
                    events.append(("csuite_synced_learning", request["task_id"]))
        ...
        # Run both sides
        await asyncio.wait_for(asyncio.gather(run_csuite_side(), run_nexus_side()), timeout=10.0)
        ...
        # ag3ntwerk processed
        assert "csuite_received_request" in event_types
        assert "csuite_sent_response" in event_types
        assert "csuite_synced_learning" in event_types
        ...
        csuite_received_idx = event_types.index("csuite_received_request")
        assert nexus_sent_idx < csuite_received_idx
    ...
    async def test_multiple_executives_flow(self, clean_redis):
        ...
        await asyncio.wait_for(asyncio.gather(csuite_listener(), nexus_sender()), timeout=10.0)
...
class TestHealthMonitoring:
    ...
    async def test_health_status_publishing(self, clean_redis):
        ...
        await asyncio.wait_for(asyncio.gather(nexus_listener(), csuite_publisher()), timeout=5.0)
