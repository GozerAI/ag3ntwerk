...
if os.path.exists(_executor_path):
    _executor_module = _import_from_path("test_exec_agentwerk", _executor_path)
...
class TestAutonomousExecutorAgentWerkRouting:
    ...
    def test_executor_accepts_agentwerk_bridge(self):
        """Executor should accept agentwerk_bridge parameter."""
        mock_bridge = MagicMock()
        executor = AutonomousExecutor(agentwerk_bridge=mock_bridge)
        assert executor._agentwerk_bridge is mock_bridge

    def test_set_agentwerk_bridge(self):
        """set_agentwerk_bridge should set the bridge."""
        executor = AutonomousExecutor()
        mock_bridge = MagicMock()

        executor.set_agentwerk_bridge(mock_bridge)

        assert executor._agentwerk_bridge is mock_bridge


class TestExecuteViaAgentWerk:
    """Test _execute_via_agentwerk method."""

    @pytest.mark.asyncio
    async def test_execute_viaagentwerk_no_bridge_falls_back(self):
        ...
        result = await executor._execute_via_agentwerk(item, "ag3ntwerk:Forge", {})

        # Should have fallen back to expert_router
        executor._execute_expert_router.assert_called_once()

    @pytest.mark.asyncio
    async def test_execute_viaagentwerk_disconnected_bridge_falls_back(self):
    ...
    @pytest.mark.asyncio
    async def test_execute_viaagentwerk_extracts_agent_code(self):
        ...
        try:
            await asyncio.wait_for(
                executor._execute_via_agentwerk(item, "ag3ntwerk:Echo", {}), timeout=0.5
            )
    ...
    @pytest.mark.asyncio
    async def test_execute_viaagentwerk_timeout_returns_error(self):
        ...
        assert result["agentwerk_execution"] is True
...
class TestExecuteRouting:
    ...
    @pytest.mark.asyncio
    async def test_execute_routesagentwerk_executor(self):
        """execute() should route ag3ntwerk:* to _execute_via_agentwerk."""
        executor = AutonomousExecutor()

        # Mock _execute_via_agentwerk
        executor._execute_via_agentwerk = AsyncMock(
            return_value={
                "agentwerk_execution": True,
                "success": True,
                "output": {"test": "result"},
                "confidence": 0.9,
            }
        ...
        executor._execute_via_agentwerk.assert_called_once()
        assert result.success is True

    @pytest.mark.asyncio
    async def test_execute_nonagentwerk_routes_normally(self):
