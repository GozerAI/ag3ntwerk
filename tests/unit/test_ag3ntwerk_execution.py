...
if os.path.exists(_executor_path):
    _executor_module = _import_from_path("test_exec_agent", _executor_path)
...
class TestAutonomousExecutorAgentRouting:
    ...
    def test_executor_accepts_agent_bridge(self):
        """Executor should accept agent_bridge parameter."""
        mock_bridge = MagicMock()
        executor = AutonomousExecutor(agent_bridge=mock_bridge)
        assert executor.agent_bridge_ref is mock_bridge

    def test_setagent_bridge_ref(self):
        """setagent_bridge_ref should set the bridge."""
        executor = AutonomousExecutor()
        mock_bridge = MagicMock()

        executor.setagent_bridge_ref(mock_bridge)

        assert executor.agent_bridge_ref is mock_bridge


class TestExecuteViaAgent:
    """Test _execute_via_agent method."""

    @pytest.mark.asyncio
    async def test_execute_via_agent_no_bridge_falls_back(self):
        ...
        result = await executor._execute_via_agent(item, "ag3ntwerk:Forge", {})

        # Should have fallen back to expert_router
        executor._execute_expert_router.assert_called_once()

    @pytest.mark.asyncio
    async def test_execute_via_agent_disconnected_bridge_falls_back(self):
    ...
    @pytest.mark.asyncio
    async def test_execute_via_agent_extracts_agent_code(self):
        ...
        try:
            await asyncio.wait_for(
                executor._execute_via_agent(item, "ag3ntwerk:Echo", {}), timeout=0.5
            )
    ...
    @pytest.mark.asyncio
    async def test_execute_via_agent_timeout_returns_error(self):
        ...
        assert result["agent_execution"] is True
...
class TestExecuteRouting:
    ...
    @pytest.mark.asyncio
    async def test_execute_routes_agent_executor(self):
        """execute() should route ag3ntwerk:* to _execute_via_agent."""
        executor = AutonomousExecutor()

        # Mock _execute_via_agent
        executor._execute_via_agent = AsyncMock(
            return_value={
        ...
        executor._execute_via_agent.assert_called_once()
        assert result.success is True

    @pytest.mark.asyncio
    async def test_execute_non_agent_routes_normally(self):
