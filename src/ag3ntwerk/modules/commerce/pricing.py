...
class MarginAnalyzer:
    ...
    def analyze_product(self, product: Product) -> Optional[MarginAnalysis]:
        ...
        if gross_margin < 0:
            ...
            analysis.recommendations.append(
                "CRITICAL: Product is losing money. Increase price or reduce cost."
            )
    ...
    def analyze_portfolio(
        ...
        # Calculate totals
        analysis.total_revenue = sum(p.price * p.total_inventory for p in products_with_data)
        analysis.total_cost = sum((p.cost or 0) * p.total_inventory for p in products_with_data)
        ...
        if analysis.gross_margin < 30:
            analysis.recommendations.append("Overall portfolio margin below healthy threshold")
...
class PricingEngine:
    ...
    def generate_recommendations(
        ...
        for product in products:
            ...
            for variant in product.variants:
                ...
                expected_margin = (
                    ((rec_price - variant.cost) / rec_price) * 100 if rec_price > 0 else 0
                )
    ...
    def get_pricing_summary(
        ...
        total_increase = sum(r.price_change for r in recommendations if r.price_change > 0)
        total_decrease = sum(r.price_change for r in recommendations if r.price_change < 0)

        return {
            "portfolio_analysis": analysis.to_dict(),
            "recommendations_count": len(recommendations),
            "price_increase_recommendations": len(
                [r for r in recommendations if r.price_change > 0]
            ),
            "price_decrease_recommendations": len(
                [r for r in recommendations if r.price_change < 0]
            ),
