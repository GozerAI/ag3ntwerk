...
class TaskQueue:
    ...
    def list_by_owner(self, agent_code: str) -> List[ScheduledTask]:
        """List tasks by owner agent."""
        return [t for t in self._tasks.values() if t.owner_executive == agent_code]
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "by_category": {
                cat.value: len([t for t in tasks if t.category == cat]) for cat in TaskCategory
            },
            "by_priority": {
                pri.value: len([t for t in tasks if t.priority == pri]) for pri in TaskPriority
            },
        }
...
class SchedulerEngine:
    ...
    async def execute_task(self, task: ScheduledTask) -> ExecutionResult:
        ...
        try:
            ...
            task.status = (
                ...
                if task.schedule.frequency != ScheduleFrequency.ONCE
    ...
    async def _schedule_retry(
        ...
        logger.info(
            f"Scheduled retry {execution.retry_count} for task: {task.name} at {retry_time}"
        )
        ...
        while self._running:
            try:
                ...
                if consecutive_failures >= 3:
                    backoff = min(
                        self._tick_interval * (2 ** (consecutive_failures - 3)), max_backoff
                    )
                    logger.warning(
                        f"Scheduler backing off for {backoff:.1f}s after {consecutive_failures} failures"
                    )
