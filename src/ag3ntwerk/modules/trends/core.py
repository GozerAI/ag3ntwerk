...
class Trend:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "Trend":
        ...
        if "category" in data:
            trend.category = (
                TrendCategory(data["category"])
                if isinstance(data["category"], str)
                else data["category"]
            )
        if "source" in data:
            trend.source = (
                TrendSource(data["source"]) if isinstance(data["source"], str) else data["source"]
            )
        if "status" in data:
            trend.status = (
                TrendStatus(data["status"]) if isinstance(data["status"], str) else data["status"]
            )
        ...
        if "last_updated" in data and data["last_updated"]:
            if isinstance(data["last_updated"], str):
                trend.last_updated = datetime.fromisoformat(
            ...
            "signals": {
                "buy_signals": len(
                    [
                        t
                        for t in top_trends
                        if t.get_signal() in [TrendSignal.BUY, TrendSignal.STRONG_BUY]
                    ]
                ),
                ...
                "sell_signals": len(
                    [
                        t
                        for t in top_trends
                        if t.get_signal() in [TrendSignal.SELL, TrendSignal.STRONG_SELL]
                    ]
                ),
            },
        }
